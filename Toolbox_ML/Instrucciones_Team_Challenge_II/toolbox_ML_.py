...
def plot_cat_distribution(df, target_col="", categorical_cols=[], mi_threshold=0.0, normalize=False):
    """
    Esta función calcula la importancia de las variables categóricas con respecto a una variable objetivo categórica o continua utilizando la información mutua.
    
    Argumentos:
        df (DataFrame): El dataframe de pandas que contiene las variables.
        target_col (str): El nombre de la columna que representa la variable objetivo.
        categorical_cols (list): Una lista opcional de nombres de columnas que se consideran variables categóricas. Si no se proporciona, se identificarán automáticamente todas las columnas cuyos datos sean de tipo categórico.
        mi_threshold (float): Umbral de importancia de información mutua. Por defecto es 0.0.
        normalize (bool): Booleano que indica si se debe normalizar las características antes de calcular la importancia de la información mutua. Por defecto es False.
        
    Devuelve:
        tuple: Una tupla que contiene una lista de las variables categóricas seleccionadas y una lista de la importancia de la información mutua normalizada de estas variables.
    """
    if df.empty:
    ...
    if not isinstance(mi_threshold, float) or mi_threshold < 0 or mi_threshold > 1:
        print("El valor de 'mi_threshold' debe ser un float entre 0 y 1.")
        return None
    
    if categorical_cols:
        invalid_columns = [col for col in categorical_cols if col not in df.columns]
        if invalid_columns:
            print(f"Las siguientes columnas no son válidas: {', '.join(invalid_columns)}")
            return None
    ...
    if normalize:
        ...
        X_normalized = scaler.fit_transform(df[categorical_cols])

        # Calcular la importancia de la información mutua
        importance = mutual_info_classif(X_normalized, df[target_col], random_state=24)
        
        # Calcular el valor normalizado de la importancia
        total_mi = sum(importance)
        normalized_importance = [mi / total_mi for mi in importance]
    else:
        # Calcular la importancia de la información mutua sin normalización
        importance = mutual_info_classif(df[categorical_cols], df[target_col], random_state=24)
        normalized_importance = importance
      
    selected_columns = [col for col, score in zip(categorical_cols, importance) if score >= mi_threshold]
    
    if not selected_columns:
        print("No se encontraron columnas que cumplan el umbral de mutual information especificado.")
        return None
    
    for col in selected_columns:
        plt.figure(figsize=(8, 6))
        df[[col, target_col]].value_counts().unstack().plot(kind='bar', stacked=False)
    ...
    return selected_columns, normalized_importance
